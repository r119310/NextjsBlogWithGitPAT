...
export default function FavoritePosts({ posts }: {
  ...
  const filteredPosts = posts.filter((post) => favoriteTags.some((tag) => post.data.tags
    ? post.data.tags.some((postTag) => postTag === tag) : false))
  ...
  return <div className='flex flex-col gap-y-3 my-3'>
    {filteredPosts.length > 0 ?
      ...
      <TipsCard>
        ...
        <p className="mt-2">「<Link className="underline" href="/post">投稿</Link>」からお気に入りのトピックを見つけよう</p>
      </TipsCard>}
  </div>
}